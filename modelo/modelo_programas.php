...
class Modelo_Programas{
	...
	function __construct()
	{
	 ...
	 function listar_programas($idsede) {
 		...
 		function listar_combo_sedes_profesor($idProfe){
 			$sql = "SELECT DISTINCT(s.sede_id), s.sede_nombre FROM sedes s INNER JOIN programa p ON p.`idsede` = s.`sede_id` INNER JOIN distribucion_programas dp ON dp.`idPrograma` = p.`programa_id` INNER JOIN docentes d ON d.`id` = dp.`idDocente`
WHERE d.`id` = '".$idProfe."'";
			$arreglo = array();
			if($consulta = $this->conexion->conexion->query($sql)){
				while($consulta_vu = mysqli_fetch_array($consulta)) {
