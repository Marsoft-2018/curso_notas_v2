...
FROM
	...
	ON 
		es.idusuario = us.usuario_id";
 		...
 		function listar_combo_programa_profe($idsede,$idusuario) {
 				$sql = "SELECT pr.programa_id,pr.nombre_programa FROM programa pr INNER JOIN distribucion_programas dp ON dp.`idPrograma` = pr.programa_id WHERE dp.`idDocente` = '".$idusuario."' AND pr.idsede = '".$idsede."'";
			$arreglo = array();
			if($consulta = $this->conexion->conexion->query($sql)){
				while($consulta_vu = mysqli_fetch_array($consulta)) {
						$arreglo[] =$consulta_vu;
